...
const ClientDashboard = ({ userBookData, setUserBookData }) => {
  ...
  const handleRemoveBook = async (transactionId, issueStatus) => {
    try {
      ...
      setUserBookData(
        userBookData.filter((book) => book._id !== transactionId)
      )
                ...
                userBookData.map((users, index) => {
                  ...
                  return (
                    <tr key={_id}>
                      ...
                      {/* Defensively render these columns only if data exists */}
                      {typeof isReturned !== 'undefined' && (
                        <td>{returnOrNot}</td>
                      )}
                      {typeof extraCharge !== 'undefined' && (
